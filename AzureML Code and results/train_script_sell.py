...
parser.add_argument('--threshold', type=float, required=True)  # Probability threshold
parser.add_argument('--fdr_level', type=float, required=True)
parser.add_argument('--highcorr', type=float, required=True)  
...
X_tsfresh = pd.concat([X1, X2, X3, X4, X5, X6, X7, X9, X10, X11, X12, X13, X14, X15, X16],axis=1, join='inner').dropna()
