...
def label_data(df,stop_loss,take_profit,loops,symbol,print_data):
    ...
    for m1 in range(len(stop_loss)):
        b_sl = mean_candle*stop_loss[m1]
        s_sl = mean_candle*stop_loss[m1]
        for m2 in range(len(take_profit)):
            b_tp = mean_candle*take_profit[m2]
            s_tp = mean_candle*take_profit[m2]
